      ...
      <S.ModalContent $isAdminMode={isAdminMode}>
        ...
        <S.ModalTabs $isAdminMode={isAdminMode}>
          ...
          <S.ModalTabButton
            ...
            color={isAdminMode ? "#818cf8" : "#4f46e5"}
          >
            안 읽음
          </S.ModalTabButton>
        </S.ModalTabs>
          ...
          {filtered.length > 0 ? (
            filtered.map((item) => (
              ...
              >
                ...
                <S.ListContent>
                  ...
                  <S.ListMessage $isAdminMode={isAdminMode}>
                    {item.message}
                  </S.ListMessage>
                </S.ListContent>
            ...
            <div
              style={{
                padding: "3rem",
                textAlign: "center",
                color: isAdminMode ? "#475569" : "#94a3b8",
              }}
            >
              <Bell
                size={40}
...
const Header = () => {
  ...
  const recentNotifications = notifications
    .filter((n) =>
      isAdminMode ? n.targetRole === "ADMIN" : n.targetRole === "USER"
    )
    .slice(0, 5); // 최근 5개 표시
  ...
  useEffect(() => {
    if (memberId) {
      axios
        ...
        .catch((err) =>
          console.error("Failed to fetch chat rooms in Header:", err)
        );
    }
  }, [memberId, setChatRooms]);

  // 전체 안 읽은 메시지 수 계산
  const totalUnreadChatCount = chatRooms.reduce(
  ...
  const userNavItems = [
    ...
    {
      id: NavItemType.CALL_RECORDS,
      label: "통화기록",
      icon: Phone,
      path: "/app/callrecords",
    },
    {
  ...
  let activeTab = currentNavItems.find((item) =>
            ...
            <S.RightSection>
              <S.RightGroup>
                ...
                >
                  ...
                  <S.ProfileInfo>
                    <S.ProfileName $isAdminMode={isAdminMode}>
                    ...
                    {showNotifications && (
                      <S.NotiPopover $isAdminMode={isAdminMode}>
                        <S.NotiHeader $isAdminMode={isAdminMode}>
                          <span>알림</span>
                          <button onClick={() => markAllAsRead(memberId)}>
                            모두 읽음
                          </button>
                        </S.NotiHeader>
                        <S.NotiList>
